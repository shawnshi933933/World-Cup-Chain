...
export default function ParlayNew() {
  ...
  const { totalOdds, worstCaseOdds, maxPayout } = useMemo(() => {
    ...
    legs.forEach(leg => {
      ...
      // Best odds (highest)
      tOdds *= Math.max(...leg.selectedOutcomes.map(o => o.odds));
      
      // Worst odds (lowest) - only relevant if multiple outcomes selected
      wOdds *= Math.min(...leg.selectedOutcomes.map(o => o.odds));
    });
  ...
  const handleToggleOutcome = (legIndex: number, outcome: any) => {
    ...
    const newLegs = [...legs];
    newLegs[legIndex] = { ...leg, selectedOutcomes: newOutcomes };
    setLegs(newLegs);
  };
            ...
            <div className="bg-card/50 border border-border/50 rounded-xl p-4 max-h-[600px] overflow-y-auto">
              {marketsLoading ? (
                <div className="space-y-4">
                  {[1, 2, 3].map(i => <div key={i} className="h-24 bg-muted animate-pulse rounded-lg"></div>)}
                </div>
                  ...
                  return (
                    <Card key={leg.marketId} className="border-primary/20">
                      <CardHeader className="p-4 pb-2 flex flex-row items-start justify-between">
                        <CardTitle className="text-base font-bold">{leg.marketTitle}</CardTitle>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => handleRemoveLeg(index)}>
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </CardHeader>
                      <CardContent className="p-4 pt-0">
                        <div className="text-xs text-muted-foreground mb-2">选择结果 (最多2个):</div>
                        <div className="grid grid-cols-2 gap-2">
                          {market?.outcomes.map(outcome => {
                            ...
                            return (
                              ...
                              >
                                <Checkbox id={`o-${index}-${outcome.name}`} checked={isSelected} disabled={isDisabled} />
                                <Label htmlFor={`o-${index}-${outcome.name}`} className="flex-1 cursor-pointer flex justify-between items-center text-sm">
            ...
            <div className="bg-muted rounded-xl p-6 mt-6 border border-border/50">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-sm text-muted-foreground mb-1">总赔率</div>
                  <div className="text-3xl font-black text-primary">1赔{totalOdds.toFixed(2)}</div>
                  {worstCaseOdds !== totalOdds && (
                    <div className="text-xs text-muted-foreground mt-1">最低: 1赔{worstCaseOdds.toFixed(2)}</div>
                  )}
                </div>
                <div className="text-center">
                  <div className="text-sm text-muted-foreground mb-1">最终可获得</div>
          ...
          <CardContent className="space-y-6 pt-6">
            <div className="flex justify-between items-center p-4 bg-muted rounded-lg border border-border">
              ...
              <div className="text-right">
                <div className="text-sm text-muted-foreground">最终可获得</div>
            ...
            <div className="space-y-3">
              ...
              {legs.map((leg, i) => (
                <div key={i} className="flex items-center gap-4 text-sm bg-background p-3 rounded border border-border/50">
                  <div className="bg-primary/20 text-primary font-bold w-6 h-6 rounded-full flex items-center justify-center shrink-0">
                    {i+1}
                  </div>
                  <div className="flex-1">
                    <div className="font-bold">{leg.marketTitle}</div>
                    <div className="text-muted-foreground">
                      {leg.selectedOutcomes.map(o => `${o.name} (1赔${o.odds.toFixed(2)})`).join(" 或 ")}
                    </div>
                  </div>
                </div>
              ))}
